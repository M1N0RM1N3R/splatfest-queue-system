...
class ProfileEditor(discord.ui.Modal):
    def __init__(self, user_id) -> None:
        ...
        self.profile: PlayerProfile = db_handling.search("PlayerProfile", cond=lambda x: x.owner_id == user_id) or PlayerProfile(user_id)
        for field in [
            {'label': "NSO Friend Code", 'placeholder': "SW-1234-5678-9012", 'value': self.profile.friend_code, 'min_length': 12, 'max_length': 17},
            {'label': "Main classic LAN play server", 'placeholder': "joinsg.net:11453", 'value': self.profile.main_lan_server, 'min_length': 9},
            {'label': "XLink Kai username", 'placeholder': "kolkraintraining", 'value': self.profile.xtag},
            {'label': "In-game name", 'placeholder': "Kolkra", 'value': self.profile.ign, 'max_length': 10},
        ]:
...
class ProfileCog(discord.Cog):
    ...
    @root.command(name="edit", description="Open the player profile editor to edit your info.")
    ...
    async def get(self, ctx:discord.ApplicationContext, user:discord.Member=None, ephemeral: bool = True):
        ...
        if profile := db_handling.search("PlayerProfile", cond=lambda x: x.owner_id == target.id):
