...
class EmbedStyleData:
    ...
    def embed(self, **kwargs):
        return Embed(
            color=self.color, title=kwargs.pop("title", self.title), **kwargs
        ).set_thumbnail(url=f"https://img.icons8.com/color/{self.icon}.png")
...
class EmbedStyle(Enum):
    ...
    Banned = EmbedStyleData(Color.red(), "law", "Banned")
    Announcement = EmbedStyleData(Color.blue(), "commercial", "Announcement")
    Muted = EmbedStyleData(Color.yellow(), "private-emoticon", "Muted")
